...
def home():
    return (
        "For current weather use /v1/current/?city=YourCity \n"
...
def getweather():
    ...
    if city is None or tstp is None:
        return "Invalid call: city name is empty", bad_request
    ...
    index = min(len(forecasts_massive) - 1, int(tstp) - 1)
    temp = forecasts_massive[index]["day"]["avgtemp_c"]
    return json.dumps({"city": city, "unit": "celsius", "temperature": temp})
